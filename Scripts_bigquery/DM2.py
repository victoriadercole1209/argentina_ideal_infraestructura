...
DATASET_DM  = "datamart_argideal"
DATASET_DW  = "datawarehouse_argideal"

client = bigquery.Client(project=PROJECT_ID)

DM_DEFS = {
    "dm_stock_ventas_distribuidor": {
        "sql": f"""
        CREATE OR REPLACE TABLE `{PROJECT_ID}.{DATASET_DM}.dm_stock_ventas_distribuidor`
        PARTITION BY fecha
        ...
        """,
        "destination": f"{PROJECT_ID}.{DATASET_DM}.dm_stock_ventas_distribuidor",
    },
    "dm_stock_resumen": {
        "sql": f"""
        CREATE OR REPLACE TABLE `{PROJECT_ID}.{DATASET_DM}.dm_stock_resumen`
        AS
        SELECT
          fecha                  AS fecha,
          id_producto,
        ...
        """,
        "destination": f"{PROJECT_ID}.{DATASET_DM}.dm_stock_resumen",
    },
}
